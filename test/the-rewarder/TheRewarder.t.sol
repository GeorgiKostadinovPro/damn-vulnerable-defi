...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        ...
        uint dvtClaimsCount = TOTAL_DVT_DISTRIBUTION_AMOUNT / playerDvtAmount;

        uint wethClaimsCount = TOTAL_WETH_DISTRIBUTION_AMOUNT /
            playerWethAmount;

        uint totalClaimsCount = dvtClaimsCount + wethClaimsCount;

        IERC20[] memory tokensToClaim = new IERC20[](2);
        tokensToClaim[0] = IERC20(address(dvt));
        tokensToClaim[1] = IERC20(address(weth));

        Claim[] memory claims = new Claim[](totalClaimsCount);
